...
const MainC = () => {
  ...
  const getProducts = async (categoria, page, termino = "") => {
    try {
      let response;
      ...
      response = await clienteAxios.get("/productos", {
        params: { categoria, skip, limit, page, termino },
      });
  ...
  const handleCategoryClick = (categoria) => {
    ...
    getProducts(categoria, 1, searchTerm);
  };

  useEffect(() => {
    getProducts(selectedCategory, 1, searchTerm);
